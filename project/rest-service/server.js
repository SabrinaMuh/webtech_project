    ...
    if (!token) return response.status(401).send({ error: "UNAUTHORIZED", reason: "No token provided" });
      jwt.verify(token, "SECRET", (err, user) => {
        if (err) return response.status(403).send({ error: "UNAUTHORIZED", reason: "Invalid token" });
         ...
         order = pool.query("select * from orders where orders.paymenttoken = $1", [token], (error, results) => {
            if(error){
                response.status(404).send(error);
            }
            if(results.rowCount == 0){
            ...
            orderedItems = pool.query("select o.ordereditemsid, o.itemid, o.quantity, o.status, o.orderid, o.orderdate, i.title from ordereditems o, items i where orderid = $1 and i.itemid = o.itemid", [orderID], (error, results2) => {
                if(error){
                    response.status(404).send(error);
                    console.log(error);
                }
...
const loadReviews = function () {
    return new Promise((resolve, reject) => {
        pool.query(`select * from reviews order by reviewdate desc`, (err, res) => {
            if(err) {
                reject(err);
            } else {
                resolve(res);
